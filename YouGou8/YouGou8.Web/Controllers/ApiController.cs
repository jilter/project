...
namespace YouGou8.Web.Controllers
{
    public class ApiController : Controller
    {
        ...
        public ActionResult GetList(string name, string types = "0",int stype=1, int page = 1, int size = 5)
        {
            int totalPage = 0;
            int totalCount = 0;
            List<Product_MY> list = new List<Product_MY>();
            if (stype == 1)
            {
            ...
            {
                return Json(new { code = 0, data = new List<Product_MY>(), total = totalPage }, JsonRequestBehavior.AllowGet);
            }
            else
            {
                ...
                foreach (var item in list)
                {
                    objList.Add(new
                    {
                        ...
                        price = item.PPrice - item.CouponMoney - item.PRedPack,
                        coupon = item.CouponMoney,
                        sales = item.PSales,
                        redpack = item.PRedPack
                    });
                }
